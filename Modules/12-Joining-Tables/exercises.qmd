---
title: "Joining Tables"
format: 
  gfm:
    output-file: "README"
    output-ext: "md"
editor: visual
---

The exercises in this document follow along closely with an [Episode 50](https://github.com/carpentries-incubator/bioc-intro/blob/685e808d18403e7d4cb5cfd654f21c1134e173d7/_episodes_rmd/50-joining-tables.Rmd) of a pilot course titled [Intro to data analysis with R and Bioconductor](https://carpentries-incubator.github.io/bioc-intro/index.html), provided by the [Software Carpentry](https://software-carpentry.org/) community under the creative commons license, [CC-BY 4.0](https://creativecommons.org/licenses/by/4.0/). For more information see [The Carpentries](https://carpentries.org/).

::: callout-tip
## Key Points

-   Join tables of data in R
:::

```{r}
#| include: false
if(FALSE)
{
    # use this code to install the package we're using for this lesson
    BiocManager::install("UCLouvain-CBIO/rWSBIM1207")
}
library("rWSBIM1207")
data(jdf)
```

#### Challenge 1

Using the `full_join` function, join tables `jdf4` and `jdf5`. What has happened for observations `P26039` and `P02468`?

<!-- Challenge 1 answer begin -->

```{r}
# Insert code here
```

<!-- Challenge 1 answer end -->

#### Challenge 2

Join tables `jdf4` and `jdf5`, keeping only observations in `jdf4`.

<!-- Challenge 2 answer begin -->

```{r}
# Insert code here
```

<!-- Challenge 2 answer end -->

#### Challenge 3

Join tables `jdf4` and `jdf5`, keeping only observations in `jdf5`.

<!-- Challenge 3 answer begin -->

```{r}
# Insert code here
```

<!-- Challenge 3 answer end -->

#### Challenge 4

Join tables `jdf4` and `jdf5`, keeping observations observed in both tables.

<!-- Challenge 4 answer begin -->

```{r}
# Insert code here
```

<!-- Challenge 4 answer end -->

#### Challenge 5

Given the inner join,

```{r}
inner_join(jdf6, jdf7)
```

interpret the result, where both tables have duplicated keys.

<!-- Challenge 5 answer begin -->

*insert answer here*

<!-- Challenge 5 answer end -->
